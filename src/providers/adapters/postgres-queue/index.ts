/**
 * `createPostgresQueue` — the production `QueueProvider`: a durable queue on
 * `queue_jobs` (migration 013, `src/db/migrate.ts` — see that migration's doc
 * for the schema rationale), reusing the Supabase Postgres every deployment
 * already provisions rather than adding Vercel Queues (still beta) as a
 * second durable-work dependency. `enqueue` implements the `QueueProvider`
 * interface (`src/providers/queue.ts`); `drainOnce` is the poll-drain side
 * that interface deliberately doesn't model, since Postgres cannot push — a
 * Vercel Cron tick calls it to pull and process one bounded batch.
 *
 * Per `src/providers/README.md`'s adapter-boundary rule, this module is wired
 * in only at the composition root — engine code (`src/mail`, `src/api`,
 * `src/store`) never imports it directly, only the `QueueProvider` type.
 *
 * ## The enqueue-commits-before-ack invariant
 *
 * `src/api/gmail-webhook.ts` acks the inbound Pub/Sub push only after
 * `deps.queue.enqueue(...)` resolves. `enqueue` is a single durable `INSERT`
 * — under `src/db/client.ts`'s `Queryable` contract a single statement is its
 * own implicitly-committed unit, so the promise resolves once Postgres has
 * durably committed the row, never before. If the webhook process dies first,
 * nothing was enqueued and the caller never observed success, so Pub/Sub
 * redelivers. There is no window in which an ack covers a job that did not
 * commit.
 *
 * ## Lease model: `run_after` + `locked_until` = "eligible" + "leased"
 *
 * A job is claimable when `dead_lettered_at IS NULL` (not terminal),
 * `run_after <= now()` (its delay/backoff elapsed), AND `locked_until IS NULL
 * OR locked_until < now()` (unleased, or a prior lease expired). `drainOnce`'s
 * claim is one atomic `UPDATE ... WHERE id IN (SELECT ... FOR UPDATE SKIP
 * LOCKED LIMIT $batch) RETURNING *`: `FOR UPDATE SKIP LOCKED` means two
 * concurrent `drainOnce` calls never claim the same row — the second skips
 * what the first locked, the Postgres-native substitute for a platform
 * queue's visibility timeout. The same statement bumps `attempts`, counted at
 * CLAIM time rather than outcome time, so a handler that crashes or times out
 * still counts against the retry ceiling instead of retrying free forever.
 *
 * ## Stale-outcome fence: `attempts` as an optimistic-concurrency generation
 *
 * `drainOnce` leases a whole batch upfront, then processes rows one at a
 * time. If a handler run outlives `leaseMs`, the row becomes claimable while
 * this worker is still on it, and a concurrent `drainOnce` can reclaim it,
 * bumping `attempts` again. Without a guard, THIS worker's later outcome
 * write still matches `WHERE id = $1` — so a stale ack would DELETE, or a
 * stale retry would reset `locked_until`/`run_after` on, a row another worker
 * now owns: at best redundant processing, at worst (a stale ack landing on a
 * row the new owner had rescheduled) a DROPPED job.
 *
 * So every outcome write — the ack `DELETE`, the retry `UPDATE`, and
 * `deadLetterJob`'s `UPDATE` — carries `AND attempts = <the value this worker
 * claimed>` and `RETURNING id`. A reclaim has since bumped `attempts` past
 * that value, so a stale write matches 0 rows and touches nothing. Zero
 * affected rows is treated as "reclaimed, not mine": logged
 * (`queue_stale_skip`), counted as {@link DrainReport.staleSkipped}, and NOT
 * counted as ack/retry/deadLetter. This holds the never-drop invariant
 * (charter §2) structurally, independent of handler idempotency. The
 ...
 * ## Dedupe
 *
 * `enqueue`'s `INSERT ... ON CONFLICT (topic, dedupe_key) WHERE dedupe_key IS
 * NOT NULL AND dead_lettered_at IS NULL DO NOTHING` targets migration 013's
 * `queue_jobs_topic_dedupe_key` partial unique index. A duplicate enqueue
 * sharing `(topic, dedupeKey)` with a still-live job is silently suppressed,
 * matching `EnqueueOptions.dedupeKey`'s contract. An omitted `dedupeKey` binds
 * `NULL`, which never conflicts against a unique index, so every no-key
 * enqueue always inserts.
 *
 * ## Backoff
 *
 * A `retry` outcome (explicit, or a caught throw) reschedules with exponential
 * backoff: `min((result.backoffSeconds ?? baseBackoffSeconds) * 2 ^ (attempts
 * - 1), maxBackoffSeconds)`. A handler's own `backoffSeconds` hint
 * (`QueueHandlerResult`) becomes the series' STARTING point rather than the
 * adapter's default, so a handler signaling "rate-limited, try again soon"
 * still backs off further on each subsequent failure instead of retrying at
 * the same short delay forever.
 *
 * ## A throw is a retry with no hint
 *
 * Per `QueueMessageHandler`'s contract, every handler invocation is wrapped in
 * try/catch. A caught throw becomes `{ kind: 'retry' }` with no
 * `backoffSeconds` (falling back to `baseBackoffSeconds`) and `last_error` set
 * to the caught error's message; an explicit `{ kind: 'retry' }` carries no
 * message, so `last_error` stays `null`.
 *
 * ## Dead-lettering: retried-out or explicit, but always retained
 *
 * A `deadLetter` result, OR a `retry` whose `attempts` reached the effective
 * ceiling, sets `dead_lettered_at` and clears `locked_until` — the row is
 * NEVER deleted (invariant #1: never silently drop a job). This mirrors
 * `inbound_deliveries.status = 'dead-letter'` (migration 012): a poison job is
 * parked, visible via {@link PostgresQueue.getStats}'s `deadLettered` count,
 * and available for manual review. Migration 013's `queue_jobs_ready_idx`
 * excludes dead-lettered rows, so one is never re-claimed.
 *
 * ## The retry ceiling is a call-level knob, not the row's `max_attempts`
 *
 * `queue_jobs.max_attempts` exists and defaults to 5 on every row, but nothing
 * here ever writes anything else to it. The dead-letter-vs-retry decision
 * compares a claimed row's `attempts` against `drainOnce`'s own `maxAttempts`
 * option (falling back to this factory's `options.maxAttempts`, then 5) — NOT
 * the row's column. A deliberate judgment call: it makes the ceiling an
 * operational knob adjustable for one drain pass without a backfill, and keeps
 * "retry past maxAttempts dead-letters" simply testable, at the cost of
 * `max_attempts` being schema head-room rather than a live per-job override.
 * Wiring a per-job ceiling through `EnqueueOptions` later would make the column
 * authoritative and this option its fallback.
 *
 * ## `topic IN (...)`, not `topic = ANY($array)`
 *
 * The claim query restricts to topics with a registered handler via a
 * dynamically-sized `IN ($n, $n+1, ...)` list — one bind parameter per topic,
 * the same style `src/store/conversations.ts`'s `listConversations` uses for
 * its dynamic `WHERE`. `src/db/client.ts`'s `SqlValue` is deliberately narrow
 * and does not include arrays; widening that shared seam for one call site was
 * judged out of scope. `IN (...)` is semantically identical to `= ANY(array)`
 * for a non-empty list, which the caller always has: `drainOnce` returns early
 * when no handlers are registered, before this query is built.
 */
