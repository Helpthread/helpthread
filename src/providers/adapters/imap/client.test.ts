...
describe('imapImplicitTlsForPort', () => {
  ...
  // The rule deliberately ignores the shared `secure` flag. A middle revision
  // used `port === 993 || secure`, which fixed non-standard implicit-TLS ports
  // and broke a COMMON mixed-mode setup: IMAP 143 (STARTTLS) alongside SMTP
  // 465 (implicit) carries `secure: true`, which forced a TLS handshake
  // against a server expecting a plaintext IMAP greeting. See the function's
  // doc for all three rules and why this one's failure is the rarest.
  it('does not let an SMTPS-driven secure:true force implicit TLS on IMAP 143', () => {
    // IMAP 143 + SMTP 465 → secure:true describes the SMTP leg only.
    expect(imapImplicitTlsForPort(143)).toBe(false)
  })

  it('the known, documented gap: a non-standard implicit-TLS IMAP port gets STARTTLS', () => {
    // Wrong for an operator running implicit TLS on 1993, and accepted as the
    // least-bad approximation until per-leg TLS fields exist. Fails loudly at
    // connect time — never a silent cleartext downgrade.
