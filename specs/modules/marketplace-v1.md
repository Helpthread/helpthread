...
5. **Partial refund first (the no-op case)**: issue a test-mode *partial* refund on the
   captured charge → verify the license state is **unchanged**, downloads and
   update-check keep working, and the hosted instance is untouched. This is the half of
   the HT-86 rule that a full-refund-only test would silently miss: a webhook that
   ignored the cumulative-amount comparison would pass every other step of this plan.
6. **Refund (full)**: trigger a test-mode refund for the **full** captured amount
   (`charge.refunded`, cumulative refunded equal to captured) → verify license flips to
...
7. **Dispute lifecycle**, via Stripe test mode's dispute-simulation test cards —
...
8. **Revoke**: simulate a `revoked` state via *manual* admin action only (standing in
...
9. **Exit criteria**: all of the above — active, lapsed, frozen, refunded, and revoked,
...
|---|---|---|---|
| Commercial module license text (replaces the `All rights reserved` placeholder in module repos) — **DRAFTED 2026-07-19**: `legal/module-commercial-license.md` (this repo), pending TJ counsel sign-off + adoption into each module repo | Drafted; adoption pending | Before Stripe flips to live mode / before the marketplace takes real money | Does **not** gate HT-82's test-mode dogfood — Resonant IQ as its own test-mode customer needs no finished license text, only a placeholder |
| Terms of sale — including the refund policy (§2's `refunded` state, §5.3's decommission-after-refund), the refund window (**14 days**) and config-export grace period (**7 days**) (§5.3; the day figures are confirmed, §10.11 — only the customer-facing policy *wording* remains on this gate), and the dispute-handling policy (§2's `frozen` state): this spec pins the entitlement and hosting-lifecycle *mechanics* and the exact windows; the customer-facing policy language is counsel's/TJ's call — **DRAFTED 2026-07-19**: `legal/terms-of-sale.md` in the marketplace repo, pending TJ counsel sign-off + publication on the store | Drafted; sign-off + publication pending | Before Stripe flips to live mode / before the marketplace takes real money | Same — pre-revenue, not pre-dogfood (test-mode dogfood uses the confirmed figures) |
| Privacy policy for the store — **DRAFTED 2026-07-19**: `legal/privacy-policy.md` in the marketplace repo (purchaser-PII scope, subprocessors incl. Resend, category retention), pending sign-off + publication | Drafted; sign-off + publication pending | Same gate (real customer PII starts flowing at first live charge) | Same |
| **Managed-hosting terms & data-handling disclosure** (new in v1.1) — because a Resonant IQ-hosted module reads the operator's conversation data on Resonant IQ infrastructure to do its job, managed hosting is the charter's "hosted convenience services" (§3) made real, and it needs its own disclosure/DPA-style terms the operator consents to at enrollment (§3e). This is distinct from the store privacy policy, which covers *purchaser* PII; this covers the *operator's end-customers'* conversation data flowing through a hosted module — **DRAFTED 2026-07-19**: `legal/managed-hosting-terms.md` in the marketplace repo (incl. Article-28-shaped Data Processing Terms and the model-provider conduit clause), pending sign-off; consented at enrollment when managed hosting ships | Drafted; sign-off pending | Same gate — real operator data starts flowing through a hosted module at first live managed install | Not pre-dogfood — Resonant IQ hosting its own module for its own desk raises no third-party data question. **The charter §2 own-your-data reconciliation flagged at §10.13 is now RESOLVED — charter §2 was amended the same day (HT-5/HT-82); the managed-hosting data-handling terms/disclosure themselves remain on this gate** |
...
- **npm distribution.** v1 ships tarball distribution only (operator Vercel deploy —
  the "Resonant IQ-hosted deploy for managed installs" half was rescinded 2026-07-20,
  see the banner at the top of this document). Publishing modules as installable npm
  packages is additive, later. *(The "per TJ's decision" citation previously on this
  line was unsupported and has been removed.)*
...
**Reading this list — corrected 2026-07-20 (HT-100).** An earlier version of this note
declared every item carrying a dated disposition (SIGNED OFF, CONFIRMED, DECIDED,
DIRECTION APPROVED) **closed**, and instructed readers that any "confirm"/"needs sign-off"
wording inside them was *historical*.

**Do not read it that way.** An audit on 2026-07-20 established that several of those
dispositions record decisions TJ never made — they were generated by an assistant and
then written up as settled, in some cases attributed to him by name. Specifically
unresolved: the annual-subscription-per-deployment pricing model, the refund and dispute
posture, and one-license-one-domain.

Treat a dated disposition as **a claim to verify, not a closure**, until TJ has
confirmed it in his own words. The hedging language inside these items is current, not
historical. Items §10.1, §10.2, §10.4, §10.5, and §10.7 remain open in the ordinary
sense; the rest are open pending confirmation.
...
6. **Reactivation-after-lapse restores full latest access with no back-charge — SIGNED OFF 2026-07-19 (see the Resolved block above).** (§2)
   Original reasoning retained: the alternative (pro-rated back-charge for skipped releases)
...
8. **Bundle / multi-module pricing v1: none — CONFIRMED 2026-07-19 (see the Resolved block above; bulk-domain purchase is the noted v1.1 candidate shape).** (§9) Original reasoning retained: recommendation is no
...
9. **Which tax jurisdictions to register Stripe Tax for — DECIDED 2026-07-19 (US home-state nexus at live-mode flip; see the Resolved block above).** Before flipping to live
   mode (§8) — not an engineering decision, but needed before that gate closes;
   flagged here so it isn't discovered late.
10. **Hosting control plane — co-located vs. fully separate (§3d) — CONFIRMED 2026-07-19 as co-located (see the Resolved block above).** Recommended
    ...
    store surface — holds.
11. **Refund window and config-export grace period — exact figures (§3d/§5.3/§8) — CONFIRMED 2026-07-19; policy *wording* still counsel-drafted at the §8 gate.**
    ...
    figures.
12. **Scoped per-desk provisioning credential — a new substrate requirement (§3e) — DIRECTION APPROVED 2026-07-19; substrate ticket to file (see the Resolved block above).**
