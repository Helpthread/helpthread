...
export function CommandMenu({ items = [], placeholder = "Search saved replies…", onPick, inline,
  ...
  return (
    <div style={{ width, background: "var(--ht-surface)", border: "1px solid var(--ht-border)",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 12px",
        ...
        <span style={{ fontFamily: "var(--ht-mono)", fontSize: 11, color: "var(--ht-ink-dim)",
          border: "1px solid var(--ht-border)", borderBottomWidth: 2, borderRadius: "var(--ht-radius-sm)",
          padding: "1px 6px" }}>
          esc
        </span>
      </div>
      {filtered.length === 0 ? (
        <div style={{ padding: "34px 20px 30px", textAlign: "center" }}>
          <div style={{ fontSize: 14.5, fontWeight: 600, color: "var(--ht-ink)" }}>No matching replies</div>
          <div style={{ margin: "6px auto 0", maxWidth: 220, fontSize: 12.5, lineHeight: 1.5,
        ...
        <div ref={listRef} style={{ maxHeight: 244, overflowY: "auto", padding: 5 }}>
          {filtered.map((it, i) => (
            <button key={it.label} type="button" onClick={() => onPick?.(it)} onMouseMove={() => setHi(i)}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ display: "inline-flex", color: i === hi ? "var(--ht-accent)" : "var(--ht-ink-dim)" }}>
                  {IconReply(14)}
                </span>
              ...
              {it.snippet ? (
                <div style={{ margin: "2px 0 0 24px", fontSize: 12, lineHeight: 1.4, color: "var(--ht-ink-muted)",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "7px 12px",
        borderTop: "1px solid var(--ht-divider)", fontSize: 11, color: "var(--ht-ink-dim)" }}>
