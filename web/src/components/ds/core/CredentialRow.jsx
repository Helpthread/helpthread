...
export function CredentialRow({ cred, demo, onRename, onRevoke, first }) {
  ...
  return (
    <div onMouseEnter={() => !demo && setHover(true)} onMouseLeave={() => !demo && setHover(false)}
      ...
      <div style={{ width: 38, height: 38, flexShrink: 0, display: "flex", alignItems: "center",
        justifyContent: "center", borderRadius: "var(--ht-radius-sm)", background: "var(--ht-accent-soft)",
        color: "var(--ht-accent)" }}>
        {IconKey(20)}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        {renaming ? (
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            ...
            />
            <button type="button" onClick={() => { setRenaming(false); onRename?.(cred, name); }}
          ...
          <div style={{ fontSize: 13.5, fontWeight: 600, color: "var(--ht-ink)", whiteSpace: "nowrap",
            overflow: "hidden", textOverflow: "ellipsis" }}>
            {name}
          </div>
        )}
        {!renaming ? (
          <div style={{ marginTop: 3, fontSize: 12, color: "var(--ht-ink-dim)", fontVariantNumeric: "tabular-nums" }}>
      ...
      {!renaming ? (
        <div style={{ display: "flex", alignItems: "center", gap: 4, flexShrink: 0,
          opacity: hover || armed || demo ? 1 : 0, transition: "opacity .12s" }}>
          {armed ? (
            <button type="button" onClick={arm}
              style={{ font: "inherit", fontSize: 12.5, fontWeight: 600, color: "var(--ht-surface)",
                background: "var(--ht-critical)",
                border: "1px solid color-mix(in oklab, var(--ht-critical) 40%, transparent)",
                borderRadius: "var(--ht-radius-sm)", padding: "5px 12px", cursor: "pointer" }}>
              Confirm revoke
            </button>
          ) : (
            <>
              ...
              <button type="button" title="Revoke" onClick={arm}
                style={{ width: 30, height: 30, display: "inline-flex", alignItems: "center",
                  justifyContent: "center", color: "var(--ht-critical)", background: "none", border: "none",
                  borderRadius: "var(--ht-radius-sm)", cursor: "pointer" }}
                onMouseEnter={(e) => { e.currentTarget.style.background = "var(--ht-critical-soft)"; }}
                onMouseLeave={(e) => { e.currentTarget.style.background = "none"; }}>
                {IconTrash(15)}
              </button>
            </>
...
export function PasskeyList({ creds = [], empty, onAdd }) {
  ...
  return (
    <div style={{ border: "1px solid var(--ht-divider)", borderRadius: "var(--ht-radius-md)",
      background: "var(--ht-surface)", overflow: "hidden" }}>
      {empty || creds.length === 0 ? (
        <div style={{ padding: "40px 24px 34px", textAlign: "center" }}>
          <div style={{ width: 44, height: 44, margin: "0 auto 14px", display: "flex", alignItems: "center",
            justifyContent: "center", borderRadius: "var(--ht-radius-md)", background: "var(--ht-surface-2)",
            color: "var(--ht-ink-dim)" }}>
            {IconKey(24)}
          </div>
          <div style={{ fontFamily: "var(--ht-display)", fontSize: 18, fontWeight: 600, color: "var(--ht-ink)" }}>
            No passkeys yet
          </div>
          <div style={{ margin: "7px auto 18px", maxWidth: 320, fontSize: 13.5, lineHeight: 1.6,
        ...
        <>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", padding: "12px 14px",
            borderTop: "1px solid var(--ht-divider)", background: "var(--ht-bg)" }}>
            {addBtn}
          </div>
        </>
      )}
