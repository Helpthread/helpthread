...
export function SplitButton({ label = "Send", options = [], variant = "primary", loading = false,
  ...
  return (
    <div style={{ position: "relative", display: "inline-flex",
      ...
      <button type="button" disabled={isDisabled} {...focMain}
        ...
        {loading ? (
          <span style={{ width: 14, height: 14, borderRadius: "50%",
            ...
            borderTopColor: "var(--ht-on-accent)", animation: "ht-spin .7s linear infinite" }} />
        ) : null}
        <span>{loading ? "Sending…" : label}</span>
      </button>
      <button type="button" disabled={isDisabled} {...focCaret} title={`${label} options`}
      ...
      {open && options.length ? (
        <>
          ...
          <div style={{ position: "absolute", right: 0, top: 40, zIndex: 40, minWidth: 200,
            background: "var(--ht-surface)", border: "1px solid var(--ht-border)",
            borderRadius: "var(--ht-radius-md)", boxShadow: "var(--ht-shadow-md)", padding: 5,
            animation: "ht-rise .16s ease-out" }}>
            {options.map((o, i) => (
              <MenuItem key={o.label ?? i} icon={o.icon}
                onClick={() => { setOpen(false); onAction?.(o); }}>
                {o.label}
              </MenuItem>
            ))}
          </div>
